...
class DatagramIterator {
  ...
  public:
    ...
    // read_field interprets the data as a value for the Field in native endianness.
    std::vector<uint8_t> read_field(const Field *field);
    void read_field(const Field *field, std::vector<uint8_t>& buffer);

    // read_dtype interprets the data as a value for the DistributedType in native endianness.
    std::vector<uint8_t> read_dtype(const DistributedType *dtype);
    void read_dtype(const DistributedType *dtype, std::vector<uint8_t>& buffer);
    ...
    void skip_field(const Field *field);
